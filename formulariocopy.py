...
def populate_tree(tree, sql):
    ...
    tree.delete(*tree.get_children())     # clear
...
def AgregaVenta(tree, t1, t2, t3, t4, t5, t6, t7):
    try:
        ...
        total = float(t7.get("1.0", tk.END).strip())

        fecha_val = t6.get("1.0", tk.END)
        fecha = parse_fecha_usuario(fecha_val)
        ...
        cur.execute(
            ...
            (id_cliente, id_lente, id_usuario, total, fecha)
        )
...
def ModificaVenta(tree, t1, t2, t3, t4, t5, t6, t7):
    try:
        id_venta = int(t2.get("1.0", tk.END).strip())
        id_cliente = int(t3.get("1.0", tk.END).strip())
        id_lente = int(t4.get("1.0", tk.END).strip())
        id_usuario = int(t5.get("1.0", tk.END).strip())
        total = float(t7.get("1.0", tk.END).strip())
        ...
        cur.execute(
            """UPDATE ventas SET 
               ...
               fechahora = %s 
               WHERE id_venta = %s""",
            (id_cliente, id_lente, id_usuario, total, fecha, id_venta)
        )
...
def Ventas():
    ...
    b2 = tk.Button(ventas, text="Agregar", width=10, height=1, command=lambda:
                   AgregaVenta(tree, t1, t2, t3, t4, t5, t7, t6))
    b2.place(x=10, y=360)
...
def Clientes():
    ...
    b2 = tk.Button(clientes, text="Agregar", width=10, height=1, command=lambda:
                   AgregaVenta(tree, t1, t2, t3, t4, t5))
    b2.place(x=10, y=360)
    b3 = tk.Button(clientes, text="Eliminar", width=10, height=1, command=lambda: EliminaVenta(tree, t1, t2))
    b3.place(x=100, y=360)
    b4 = tk.Button(clientes, text="Modificar", width=10, height=1, command=lambda: ModificaVenta(tree, t1, t2, t3, t4, t5))
    b4.place(x=190, y=360)
    ...
    tree.column("4", width=90)
    tree.pack(fill="both", expand=True)
    tree.bind('<<TreeviewSelect>>', lambda e: on_tree_select(e, tree, t2, t3, t4, t5))
...
def Lentes():
    ...
    t2.place(x=10, y=330)
    t3 = tk.Text(lentes, width=8, height=1)
    t3.place(x=80, y=330)
    ...
    b1 = tk.Button(lentes, text="Buscar", width=10, height=1, command=lambda: BuscaVenta(tree, t1))
    b1.place(x=510, y=8)
    b1.focus_set()
    b2 = tk.Button(lentes, text="Agregar", width=10, height=1, command=lambda:
                   AgregaVenta(tree, t1, t2, t3, t4, t5))
    b2.place(x=10, y=360)
    b3 = tk.Button(lentes, text="Eliminar", width=10, height=1, command=lambda: EliminaVenta(tree, t1, t2))
    b3.place(x=100, y=360)
    b4 = tk.Button(lentes, text="Modificar", width=10, height=1, command=lambda: ModificaVenta(tree, t1, t2, t3, t4, t5))
    b4.place(x=190, y=360)
    b5 = tk.Button(lentes, text="Excel", width=10, height=1, command=lambda: ExcelVentasTree(tree))
    b5.place(x=280, y=360)
    b6 = tk.Button(lentes, text="Chrome", width=10, height=1, command=lambda: ChromeVentasTree(tree))
    b6.place(x=370, y=360)
    b7 = tk.Button(lentes, text="Salir", width=10, height=1, command=lambda: SalirVentas(lentes))
    b7.place(x=460, y=360)
...
def RepVentas():
    ...
    canvas = tk.Canvas(repVentas, width=400, height=300)
    canvas.place(x=10, y=50)

    vsb = Scrollbar(repVentas, orient="vertical", command=canvas.yview)
    vsb.place(x=400, y=50, height=300)
    ...
    columns = ("id_venta", "id_cliente", "id_lente",
               "id_usuario", "total", "fechahora")
    ...
    # initial load
    populate_tree(tree, "SELECT * FROM ventas")
